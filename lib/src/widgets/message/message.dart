...
class Message extends StatelessWidget {
  ...
  const Message({
    ...
    this.hideBackgroundOnCustomMessages = false,
  ...
  Widget _bubbleBuilder(
    ...
    if (hideBackgroundOnCustomMessages && message.type == types.MessageType.custom) {
      defaultMessage = _messageBuilder();
    } else {
      defaultMessage = (enlargeEmojis && hideBackgroundOnEmojiMessages)
          ? _messageBuilder()
          : Container(
  ...
  Widget build(BuildContext context) {
    ...
    final bool isCarousel = message.metadata?['is_carousel'] != null ? message.metadata!['is_carousel'] : false;
    ...
    final bubbleMargin = InheritedChatTheme.of(context).theme.bubbleMargin ??
        (bubbleRtlAlignment == BubbleRtlAlignment.left
            ? EdgeInsetsDirectional.only(
                bottom: 4,
                end: isCarousel ? 0 : (isMobile ? query.padding.right : 0),
                start: isCarousel ? 0 : (20 + (isMobile ? query.padding.left : 0)),
              )
            : EdgeInsets.only(
                bottom: 4,
                left: isCarousel ? 0 : (20 + (isMobile ? query.padding.left : 0)),
                right: isCarousel ? 0 : (isMobile ? query.padding.right : 0),
              ));
        ...
        children: [
          ...
          ConstrainedBox(
            constraints: BoxConstraints(
              maxWidth: isCarousel ? query.size.width : messageWidth.toDouble(),
            ),
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.end,
              children: [
